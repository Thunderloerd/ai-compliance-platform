...
function SidebarInner() {
  ...
  const flatNavItems = React.useMemo(() => {
    ...
    return navSections.flatMap((s) =>
      s.items.filter((item) => item.roles.includes(user.role))
    );
  }, [user]);
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-40 h-screen flex flex-col border-r border-border/50 bg-card shadow-sm dark:bg-background/95 dark:shadow-none backdrop-blur-xl transition-none",
        collapsed ? "w-16" : "w-64"
      )}
    >
                      ...
                      activeIndicator={
                        isActive ? (
                          <motion.div
                            ...
                            className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-primary rounded-r-sm"
      ...
      <div ref={collapseButtonRef} className="border-t border-border/50 p-3 shrink-0 relative">
        <Button
          ...
          className="w-full hover:bg-accent/50 transition-colors duration-150 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        ...
        createPortal(
          ...
          >
            {tooltipHref === "__collapse"
              ? "Expand sidebar"
              : flatNavItems.find((i) => i.href === tooltipHref)?.label ?? ""}
          </div>,
          document.body
        )}
    </aside>
