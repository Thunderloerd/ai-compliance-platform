...
import { FileUp, FileText, Check, X, Archive } from "lucide-react";
...
const DEFAULT_MAX_ZIP_GB = 15;
const ZIP_MAX_SIZE_STORAGE_KEY = "compliance_zip_max_size_gb";
...
export default function UploadPage() {
  ...
  const [maxZipSizeGb, setMaxZipSizeGb] = useState(DEFAULT_MAX_ZIP_GB);
  const [zipState, setZipState] = useState<"idle" | "dragging" | "uploading" | "complete" | "error">("idle");
  ...
  useEffect(() => {
    ...
    getSettings().then((s) => {
      ...
      if ("zip_upload_max_size_gb" in s) {
        const val = Number(s.zip_upload_max_size_gb) || DEFAULT_MAX_ZIP_GB;
        setMaxZipSizeGb(Math.max(1, Math.min(100, val)));
      }
    });
  }, []);

  // When backend is not connected, use max ZIP size from Settings (localStorage)
  useEffect(() => {
    if (config.apiUrl) return;
    try {
      const stored = localStorage.getItem(ZIP_MAX_SIZE_STORAGE_KEY);
      if (stored !== null) {
  ...
  const runUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      setUploadError(
        isOpenAiKey
          ? "OpenAI API key not set or invalid. Add OPENAI_API_KEY to the backend .env file and restart the server."
          : raw
      );
  ...
  const maxZipBytes = maxZipSizeGb * 1024 * 1024 * 1024;
  ...
  const handleZipDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (file.size > maxZipBytes) {
        ...
        setZipError(`File exceeds maximum size (${maxZipSizeGb} GB). Increase the limit in Settings → Policy if needed.`);
        setZipState("error");
        return;
      }
      setZipFileName(file.name);
      setZipError(null);
      setZipState("uploading");
      setTimeout(() => {
        setZipState(config.apiUrl ? "error" : "complete");
        setZipError(config.apiUrl ? "ZIP upload not implemented yet. Use PDF upload for now." : null);
      }, 800);
    },
    [maxZipBytes, maxZipSizeGb]
  );

  const handleZipDragOver = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    setZipState("dragging");
  }, []);
  ...
  const resetZipUpload = useCallback(() => {
    setZipState("idle");
    setZipFileName(null);
    setZipError(null);
  }, []);
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Upload PDF policy documents for AI-powered rule extraction
          </p>
        </div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {uploadState === "error"
                              ...
                              : uploadState === "uploading"
                                ? "Uploading & processing PDF… (may take 30–90s)"
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {uploadState === "processing"
                              ? "Almost done…"
                              : "This may take 15–60s for large PDFs."}
                          </p>
                        </div>
                      )}
                      {uploadState === "error" && (
        ...
        <Card className="overflow-hidden" glass={false} variant="elevated">
          <CardHeader>
            <h3 className="font-semibold">Upload ZIP</h3>
            <p className="text-sm text-muted-foreground">
              Bulk upload: drop a ZIP file containing multiple PDF policies
            </p>
          </CardHeader>
          <CardContent>
            <motion.div
              ...
              onClick={() => zipState === "idle" && document.getElementById("zip-input")?.click()}
              className={cn(
                ...
                zipState === "dragging" && "border-primary bg-primary/5 scale-[1.01]",
                zipState === "idle" && "border-muted-foreground/25 hover:border-primary/50 hover:bg-accent/20",
                (zipState === "uploading" || zipState === "complete" || zipState === "error") && "border-primary/50 bg-accent/10"
              )}
            >
              <input
                id="zip-input"
              ...
              {zipState === "idle" && (
                <>
                  ...
                  <p className="mt-3 font-medium">Drop ZIP here or click to upload</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Max {maxZipSizeGb} GB. Change in Settings → Policy. ZIP can contain multiple PDFs for batch processing.
                  </p>
                </>
              )}
              {zipState === "uploading" && (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm font-medium">{zipFileName}</p>
                  <p className="text-xs text-muted-foreground">Processing ZIP…</p>
                </div>
              )}
              {zipState === "complete" && (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="font-medium">ZIP received</p>
                  <p className="text-sm text-muted-foreground">{zipFileName}</p>
                  <Button onClick={resetZipUpload} variant="outline" size="sm">
                    Upload another
                  </Button>
                </div>
              )}
