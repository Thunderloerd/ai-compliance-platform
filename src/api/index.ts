...
// --- Empty / default when no API URL or fetch fails ---
...
export async function fetchDatabaseStatus(): Promise<{
  ...
  try {
    return await api.get<{ connected: boolean; host?: string; db_name?: string; username?: string; port?: string }>("/database/status");
  } catch {
    return { connected: false };
  }
}

/** Connect external company database for scanning. */
...
/** Delete a rule by id. Timeout 15s so the UI does not hang. */
export async function deleteRule(ruleId: string): Promise<void> {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    if (!res.ok) {
      let message = res.statusText || "Delete failed";
      try {
        const body = await res.json();
        if (body?.detail != null) message = typeof body.detail === "string" ? body.detail : body.detail[0]?.msg ?? body.detail[0] ?? message;
      } catch {
    ...
    clearTimeout(timeoutId);
    if (e instanceof Error && e.name === "AbortError") throw new Error("Delete timed out. Try again.");
    throw e;
  }
}

/** Update a rule's status (active or inactive). Backend: PATCH /rules/:id with { status }. */
export async function updateRuleStatus(
  ruleId: string,
  status: "active" | "inactive"
): Promise<void> {
  if (!config.apiUrl) throw new Error("API not configured");
  await api.patch(`/rules/${ruleId}`, { status });
}
...
const ASK_TIMEOUT_MS = 120_000;  // 2 min – first Ask after upload can be slow (indexing, embeddings, Groq)
...
/** RAG: Ask a natural-language question about compliance policy. Requires auth; rate limited. Timeout 2 min. */
export async function fetchPolicyAsk(query: string, policyId: number | null = null): Promise<{ answer: string }> {
  ...
  try {
    ...
    if (e instanceof Error && e.name === "AbortError") {
      throw new Error("The request took too long (over 2 min). The first question after uploading a policy can take 1–2 min—try again, or run \"Index existing policies\" and wait for it to finish.");
    }
    throw e;
  }
}
...
export async function fetchNotifications(): Promise<NotificationItem[]> {
  if (!config.apiUrl) return [];
  try {
    const list = await api.get<Array<{ id: number; type: string; title: string; body: string | null; read: boolean; created_at: string | null }>>(
      "/notifications"
    );
    return list.map((n) => ({
      id: String(n.id),
