...
def index_policy_chunks(
    ...
    """
    Chunk policy text, embed, and add to Chroma. Enforces RAG_MAX_CHUNKS_PER_POLICY.
...
def delete_policy_from_index(policy_id: int) -> None:
    """Remove all chunks for the given policy_id from Chroma."""
    pair = _get_client_and_collection()
    if not pair:
        return
    _client, collection = pair
...
def get_indexed_count() -> int:
    """Return the number of documents in the RAG collection, or 0 if unavailable."""
    pair = _get_client_and_collection()
    if not pair:
        return 0
    _client, collection = pair
...
def warmup_rag() -> None:
    """
    Pre-warm RAG: load embedding model and Chroma so first upload/reindex is fast.
    ...
    try:
        ...
        if not pair:
            return
        _client, collection = pair
        # Run a tiny query to force first encode (warms the embedding model fully)
...
def retrieve(
    ...
    """
    Embed query, similarity search in Chroma, return list of
    { "text", "policy_id", "chunk_index", "policy_name", "score" }.
    Uses dynamic top-k (smaller k when similarity is high), similarity threshold, optional re-ranking.
    If policy_id is set, filter to that policy only.
    If use_summaries_only=True (and RAG_USE_SUMMARIES), query only summary chunks to reduce tokens for broad queries.
    """
    pair = _get_client_and_collection()
    if not pair:
        return []
