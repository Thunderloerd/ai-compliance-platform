...
def init_db() -> None:
    """Create the database if missing (PostgreSQL only), then create all tables. Seed default admin if none."""
...
def _add_violation_suggested_remediation_column() -> None:
    """Add suggested_remediation column to violations if it does not exist (for existing deployments)."""
...
def _add_policy_extracted_text_column() -> None:
    """Add extracted_text column to policies if it does not exist (for existing deployments)."""
    dialect_name = engine.dialect.name
    with engine.connect() as conn:
        try:
            if dialect_name == "postgresql":
