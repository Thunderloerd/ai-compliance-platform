...
from app.models import AuditLog, Policy, Rule, ScanState, User, Violation
from app.routes.auth_routes import get_current_user

router = APIRouter(prefix="/reports", tags=["reports"])


def _log_export(db: Session, performed_by_email: str) -> None:
    """Log an export action for profile metrics (per user)."""
    db.add(
        AuditLog(
            action_type="export",
            entity_type="report",
...
def export_csv(
    ...
    _log_export(db, current_user.email)
